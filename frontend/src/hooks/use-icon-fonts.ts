...
export const useIconFonts = (): readonly [boolean, Error | null] => {
  const inGo = Constants.executionEnvironment === ExecutionEnvironment.StoreClient;
  const [loaded, error] = useFonts(inGo ? iconFontMap() : {});
  // NEVER gate the app on icon font loading. If the CDN times out or errors
  // (common on slow networks / Expo Go), swallow the error and let the app
  // render — icons fall back to tofu characters at worst, but the app
  // must not crash with an uncaught promise rejection.
